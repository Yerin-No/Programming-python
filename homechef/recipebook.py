...
class RecipeBook:
    ...
    def search_ingredient(self):
        #빈 셋(set)을 생성 -> 재료를 중복 제거해서 담은 셋(set)
        all_inderedient = set()
        #레시피북에 있는 레시피의 재료들 셋에 넣기
        for recipe in self.recipe_list:
            for ingredient in recipe.ingredient:
                all_inderedient.add(ingredient)
        #모든 재료들 보여주기
        for index, ingredient in enumerate(all_inderedient):
            print(f'{index+1}. {ingredient}')

        #찾을 재료 이름을 입력
        search_num = int(input('>>사용할 재료를 입력하세요: '))
        search_ingredient = list(all_inderedient)[search_num-1]
    ...
    def init_recipe(self):
        ...
        떡볶이.people = 2
        떡볶이.video = 'youtube'
        떡볶이.ingredient = {'떡': '200', '고추장': '100','오뎅': '100','물': '100'}
        self.recipe_list.append(떡볶이)
        카레 = Recipe('카레')
        카레.ingredient = {'카레가루': '200','감자': '200'}
        self.recipe_list.append(카레)
        파스타 = Recipe('파스타')
        파스타.contents = '맛있엇'
        파스타.ingredient = {'면':'100'}
